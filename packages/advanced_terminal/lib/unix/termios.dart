...
typedef SystemC = Int32 Function(Pointer<Utf8> command);
typedef SystemDart = int Function(Pointer<Utf8> command);
...
late final SystemDart _system;

void _initializeTermIOSLibrary() {
  if (_isInitialized) {
    ...
    _system = _stdlib.lookupFunction('system');

// store console mode settings so we can return them again as necessary
    _origTermIOSPointer = calloc<TermIOS>();
    _tcgetattr(0/*stdin*/, _origTermIOSPointer);
  } else {}
}

int system(String s) {
  final newString = s.toNativeUtf8();
  final res = _system(newString);
  calloc.free(newString);
  return res;
}